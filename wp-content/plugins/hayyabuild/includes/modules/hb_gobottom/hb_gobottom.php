 ...
 * @since      1.0.0
 * @package    hayyabuild
 * @subpackage hayyabuild/includes/modules/hb_gobottom
 * @author     ZintaThemes <>
 */
class HayyaModule_hb_gobottom
{
	/**
	...
	public function __construct() {
		...
		$this->name 		= __('Go Down', HAYYAB_BASENAME);
		$this->description 	= __('Insert go down link', HAYYAB_BASENAME);
		
		$this->params = array(
				'GoDownIcon' => array(
						'type' => 'icon',
						'heading' => __('Select icon', HAYYAB_BASENAME),
						'description' => __('Just keep it empty to set this icon ( <i class=\"fa fa-angle-double-down\"></i> ).', HAYYAB_BASENAME),
				),
                'hb_animation' => array(
                        'type' => 'checkbox',
                        'heading' => __('Animate this link ?', HAYYAB_BASENAME),
                		'value' => array('hb_animate' => __('Yes', HAYYAB_BASENAME)),
                ),
                'hb_align' => array (
                        'type' => 'dropdown',
	...
	public function admin_render($param = null) {
		$args = array(
				'GoDownIcon' => array(null => 'fa fa-angle-double-down', 'else' => $param['GoDownIcon'],),
				'hb_animation' => ' class="hb_animation"',
		);
		$html = '<div id="'.$param['id'].'" class="'.$param['class'].'" style="text-align:'.$param['hb_align'].';"><a href="#hb_afterheader" id="hb_gobottom" style="'.$param['style'].'"'.$param['hb_animation'].'> <i class="'.$param['GoDownIcon'].'" ></i> </a></div>';
