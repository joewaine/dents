...
(function( $, doc, win ) {
    ...
    $.each( optin_keys, function(i, k){
        var opt = Optins[ k ];
        if( _.isTrue( opt.settings.slide_in.display ) && !_.isTrue( Optin.cookie.get( Optin.SLIDE_IN_COOKIE_PREFIX + opt.data.optin_id ) )  ) {
        ...
        slide_in.on("show", function(){ //  on show log view
            ...
            if( _.isTrue( slidein_optin.settings.slide_in.hide_after ) ) {
                ...
                var delay_id = _.delay(function(){
                    if( !slide_in.data('prevent_hide_after') ) // if hide after is not prevented, then hide it
