(function($) {
	...
	mg_item_img_switch = function(first_init) {
		...
		if(jQuery(window).width() < safe_mg_mobile && (!mg_mobile_mode || typeof(first_init) != 'undefined')) {
			jQuery('.mg_box:not(.mg_pag_hide) .thumb').each(function() {
                jQuery(this).attr('src', jQuery(this).data('mobileurl'));
            });
		...
		if(jQuery(window).width() >= safe_mg_mobile && (mg_mobile_mode || typeof(first_init) != 'undefined')) {
			jQuery('.mg_box:not(.mg_pag_hide) .thumb').each(function() {
                jQuery(this).attr('src', jQuery(this).data('fullurl'));
            });
	...
	mg_get_w_size = function(box_id, mg_wrap_w) {
		var size = (mg_mobile_mode) ? jQuery(box_id).data('mgi_mw') : jQuery(box_id).data('mgi_w');
	...
	mg_get_h_size = function(box_id, mg_wrap_w, mg_box_w) {
		var hsize = 0;
		var size = (mg_mobile_mode) ? jQuery(box_id).data('mgi_mh') : jQuery(box_id).data('mgi_h');
		...
		// "auto" height calculation
			...
			// image aspect ratio
				...
				var ratio = parseFloat( jQuery(box_id).data('ratio') );
	...
	jQuery.fn.mg_display_boxes = function(grid_id) {
		...
		jQuery(window).trigger('mg_grid_shown', [grid_id]);
	...
	mg_remove_scrollbar = function() {
		mg_html_style = (typeof(jQuery('html').attr('style')) != 'undefined') ? jQuery('html').attr('style') : '';
		mg_body_style = (typeof(jQuery('body').attr('style')) != 'undefined') ? jQuery('body').attr('style') : '';
		
		// avoid page scrolling and maintain contents position
		var orig_page_w = jQuery(window).width();
		jQuery('html').css({
			'overflow' 		: 'hidden',
			'touch-action'	: 'none'
		});
		...
		mg_fullpage_w = jQuery(window).width();
		jQuery('html').css('margin-right', (jQuery(window).width() - orig_page_w));
	};
	...
	mg_get_item_content = function(pid, on_item_switch) {
		...
		jQuery('#mg_lb_wrap').data('item-id', pid).data('grid-id', gid);
	...
	mg_switch_item_act = function(pid) {
		...
		jQuery('#mg_lb_top_nav, .mg_side_nav, .mg_lb_nav_side_basic, #mg_top_close').fadeOut(350, function() {
			jQuery(this).remove();
		});
	...
	mg_lb_realtime_actions = function() {
		...
		mg_lb_realtime_actions_intval = setInterval(function() {
			...
			if(jQuery('#mg_lb_contents').outerHeight(true) > jQuery(window).height()) {
				...
				var diff = mg_fullpage_w - jQuery('#mg_lb_scroll_helper').outerWidth(true);
				jQuery('#mg_top_close, .mg_side_nav_next').css('right', diff);
			}
			else {
				jQuery('#mg_lb_wrap').removeClass('mg_lb_has_scroll');
				jQuery('#mg_top_close, .mg_side_nav_next').css('right', 0);
			}
			...
			if(jQuery('.mg_lb_feat_match_txt').length && jQuery('#mg_lb_contents').outerHeight(true) < jQuery(window).height() && jQuery(window).width() > 860) {
				var txt_h = jQuery('.mg_item_content').outerHeight();
				
				// remove comments height to avoid bad results
				/*if(jQuery('#mb_lb_comments_wrap').length) {
					txt_h = txt_h - jQuery('#mb_lb_comments_wrap').outerHeight('true');	
				}*/
	...
	mg_do_pagination = function(direction, pag_btn_obj) {
		...
		var tot_pag = parseInt(jQuery('#mgp_'+gid).data('tot-pag'));
		...
		setTimeout(function() {
			...
			jQuery('#mg_grid_'+gid+' .mg_box').fadeTo(0, 1);	
		...
		if(!val || val.length > 2) {
			if(val) {
				...
				jQuery('#' + grid_id+' .mg_box').not('.mg_spacer').each(function() {
					var src_attr = jQuery(this).data('mg-search').toLowerCase();
	...
	jQuery(document).ready(function() {
		...
		jQuery(document).delegate('.mg_inl_video:not(.mg_disabled)', 'click'+mg_generic_touch_event, function(e){
			if(jQuery(this).find('.thumb').length) {
				...
				if(jQuery(this).find('.mg_sh_inl_video').length) {
					...
					var autop = jQuery(this).find('iframe').data('autoplay-url');
		...
		jQuery(document).delegate('#mg_lb_video_poster, #mg_ifp_ol', 'click'+mg_generic_touch_event, function(e){
			var autop = jQuery('#mg_lb_video_poster').data('autoplay-url');
			...
			if(!on_init) {
				...
				if(subj.indexOf('mgi_') !== -1) {
					...
					if(jQuery('.mg_item_content').length) {
						...
						if(jQuery('#mg_lb_wrap').data('item-id') == val && jQuery('#mg_lb_wrap').data('grid-id') == gid) {return;}
	...
	mg_galleria_height = function(sid) {
		if( jQuery(sid).hasClass('mg_galleria_responsive')) {
			...
			return parseInt(jQuery(sid).data('slider-h'));
		}
	};
	...
	function mg_responsive_txt(gid) {
		...
		$subj.each(function() {
			if(typeof( jQuery(this).data('orig-size') ) == 'undefined') {
				jQuery(this).data('orig-size', jQuery(this).css('font-size'));
				jQuery(this).data('orig-lheight', jQuery(this).css('line-height'));
			}
			...
			jQuery(this).css('font-size', jQuery(this).data('orig-size'));
			jQuery(this).css('line-height', jQuery(this).data('orig-lheight'));
        });
