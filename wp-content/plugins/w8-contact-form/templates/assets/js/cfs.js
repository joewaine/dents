...
function getString(number_value)
{
	...
	if ((jQuery("body").html()!=undefined))
	{
		...
		if (options.hide_icon=='true') fbicon2 = '<span style="display:none" class="fbicon_left icon_'+options.icon_size+'"></span>';
		if (options.direction=='left'&&options.hide_icon=='false') 
		{
			if (options.icon_url==undefined||options.icon_url=='') fbicon2 = '<span style="left:-70px;top:'+options.vertical_distance+'%" class="fbicon_left icon_'+options.icon_size+'"></span>';
			else {
			...
			fbicon2 = '<span style="left:-'+(_width+50)+'px;top:'+options.vertical_distance+'%" class="fbcs_left"><img src="'+options.icon_url+'" /></span>';
			}
		}
		if (options.direction=='right'&&options.hide_icon=='false') 
		{
			if (options.icon_url==undefined||options.icon_url=='') fbicon2 = '<span style="right:-70px;top:'+options.vertical_distance+'%" class="fbicon_right icon_'+options.icon_size+'"></span>';
			else {
			var imgSrc = options.icon_url;
			var _width, _height;
			jQuery("<img/>").attr("src", imgSrc + "?" + new Date().getTime()).load(function() {
			...
			fbicon2 = '<span style="right:-'+(_width+50)+'px;top:'+options.vertical_distance+'%" class="fbcs_right"><img src="'+options.icon_url+'" /></span>';
			}
		}
...
"border-bottom-right-radius": "100%"});
		...
		if ( options.icon_url == undefined || options.icon_url == '' ) {
			if ( options.icon_image != "" ) {
				if ( options.direction == 'left' && options.hide_icon == 'false' ) {
					jQuery( ".cfs_hdline span" ).css( "backgroundImage", "url(" + options.plugin_directory + "/templates/assets/img/icon" + options.icon_image + "-left.png)" );
				}
				if ( options.direction == 'right' && options.hide_icon == 'false' ) {
					jQuery( ".cfs_hdline span" ).css( "backgroundImage", "url(" + options.plugin_directory + "/templates/assets/img/icon" + options.icon_image + "-right.png)" );
				}
			}
		}
		...
		jQuery("<img/>").attr("src", jQuery('.cfs_hdline span img').attr("src") + "?" + new Date().getTime()).load(function() {
			_width = this.width; 
			_height = this.height;
			jQuery('.cfs_hdline span').css("width",_width+"px");
		 });
		 }
			jQuery('.cfs_hdline span').css("margin-top","-"+jQuery('.cfs_hdline span').height()/2+"px");
		...
		if (options.direction=='left') 
		{
			jQuery('.contact-form-slider-box').css("left",'-'+(parseInt(jQuery(".contact-form-slider-box").width())+space)+'px');
			 jQuery( ".contact-form-slider-box span" ).animate({
				left: (jQuery(".contact-form-slider-box").width()+bspace)+'px'
				}, 1500, "linear", function() {
				// Animation complete.
					if (parseInt(options.shake)>0) {setInterval(function(){if (opened==false) {jQuery( ".contact-form-slider-box span" ).effect( "shake", {direction: "up"} );}},parseInt(options.shake));}
					if (options.shake=='heartbeat') jQuery(".contact-form-slider-box span").addClass("heartbeat");
				});
		}
		...
		jQuery(".contact-form-slider-box span").click(function() {
	...
	opened_slider = boxtype;
		...
		jQuery("."+jQuery(boxtype).attr("class").replace("cfs_hdline ","")+" span").removeClass("heartbeat");
	   ...
	   if (parentbox=='') 
		{
			...
			jQuery( ".contact-form-slider-box span" ).trigger( "click" );
		}
	},
	close : function()
	{
	...
	if (boxtype==undefined||boxtype=='') boxtype = opened_slider;
		...
		if (options.direction=='left') jQuery(boxtype).animate({left: "-"+(parseInt(jQuery(boxtype).width())+space)+"px"}, 1000, "easeOut"+options.animationtype,function(){if (jQuery("#twitter_timeline_cfs_hdline span").length==1) jQuery("#twitter_timeline_cfs_hdline").css("z-index","11000");});
		if (options.direction=='right') jQuery(boxtype).animate({right: "-"+(jQuery(boxtype).width())+"px"}, 1000, "easeOut"+options.animationtype,function(){if (jQuery("#twitter_timeline_cfs_hdline span").length==1) jQuery("#twitter_timeline_cfs_hdline").css("z-index","11000");});
		...
		if (jQuery(".contact-form-slider-box").length==1)
	{
		...
		if (options.direction=='left') jQuery('.contact-form-slider-box span').css("left",(parseInt(jQuery(".contact-form-slider-box").width())+bspace)+'px');
		if (options.direction=='right') jQuery('.contact-form-slider-box span').css("left",'-'+jQuery(".contact-form-slider-box span").width()+'px');
