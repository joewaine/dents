...
define( 'W8CONTACT_FORM_VERSION' , '1.5.3' );
define( 'W8CONTACT_FORM_TEXT_DOMAIN' , 'w8-contact-form' );
...
if ( ! class_exists( 'contact_form_slider' ) ) {
	class contact_form_slider {
		...
		// Localization
		load_plugin_textdomain( W8CONTACT_FORM_TEXT_DOMAIN, false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
		}
		...
		**/
		...
		$params = $contact_form_slider->get_parameters();
			...
			foreach( $custom_fields[ 0 ] as $key => $cf ) {
				if ( $cf->id == 'TRPHONE' ) {
					$custom_fields[ 0 ][ $key ]->name = __( 'Phone Number', W8CONTACT_FORM_TEXT_DOMAIN );
				}
			}
		/* params for demo end */
